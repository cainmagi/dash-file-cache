...
Description
-----------
The demo for using a background callback to prepare the data, and use the queue-based
cache to serve the data to the frontend.
"""
...
from dash_file_cache import CacheQueue
from dash_file_cache import ServiceData


app = dash.Dash("test-background-callback")
...
class Demo:
    def __init__(self) -> None:
        self.service = ServiceData(CacheQueue(1))
...
class WrappedApp:
    ...
    def load(self, qobj) -> None:
        demo = Demo()
        app.layout = demo.layout()
        demo.bind(app)
        demo.service.serve(app)
        if isinstance(demo.service.cache, CacheQueue):
            demo.service.cache.qobj = qobj
