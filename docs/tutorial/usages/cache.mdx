...
Different from `CachePlain`, this queue-based cache needs a queue object as the first argument. This queue object should be provided by a [Process Data Manager<IconExternalLink/>][link-manager].
...
When the cache is not used anymore (typically, this case happens when the program exists), the folder will be removed.
...
has no difference compared with `CachePlain` or `CacheQueue`. However, in most cases, `CacheFile` cannot be used for replacing `CachePlain` or
`CacheQueue`.
