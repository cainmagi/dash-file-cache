...
此处设置了下载器，其为一个Dash组件。属性`mitm`用来维持下载事件的激活状态。其服务对应的路由需要和接下来将要用到的`ServiceDownloader`保持一致。

与其他范例相若，此处只有`id=btn`的按钮能触发callback。Callback的返回值由布局中的`Downloader`组件处理。
