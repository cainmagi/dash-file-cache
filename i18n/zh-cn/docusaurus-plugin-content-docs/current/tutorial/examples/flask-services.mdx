...
由于不便于透过浏览器发送`POST`请求，这里提供了一个脚本，用来帮助获取文件列表中、第一个列出的
文件的缓存文件地址。该脚本由Python标准库写就。
...
令应用持续运行，并打开另一个命令行窗口。复制前一个命令行窗口中、展示的应用地址，并将该地址
按如下方式传递给脚本：
...
`POST`方法接收一个必选参数`name`和一个可选参数`download`。其中，`name`用来获取文件位于服务
器上的路径，`self.serivce.register(...)`方法则用来返回缓存地址。调用这一方法，会令文件实际
载入到缓存中。
