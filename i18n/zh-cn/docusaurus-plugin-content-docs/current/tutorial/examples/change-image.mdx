...
即使没有配置`html.Img`的默认值，刷新页面时，所展示的图片仍然会重置为第一张图。这是因为图片组件的默认值已经由callback设置了。

## 定义callback {#define-the-callbacks}

Callback的触发事件是按下按钮。根据所按按钮的ID，所载入图片的对应路径将会不同。服务方法`self.service.register(...)` 接收数据和文件信息作为输入，并返回用来访问图片的地址。
...
Callback设置成了会在应用初始化时触发一次（参见选项[`prevent_initial_call`<IconExternalLink/>][link-prevent-initial-call]）。此情况下，`prop_id`的值将是`.`，且载入的图片将是`test_image.svg`。
