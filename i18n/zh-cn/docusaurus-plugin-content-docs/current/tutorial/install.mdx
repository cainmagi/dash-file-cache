...
安装了[Git<IconExternalLink/>][link-git]。确保一些需要的软件都准备无虞后，运行下列命令安装
开发者版本：
...
对从GitHub上clone的项目做出的任何改动都将反馈到全局安装的`dash_file_cache`模块包上。

:::

## 运行测试 {#run-the-tests}

要检查此项目是否兼容当前设备，请确保已经安装了[Git<IconExternalLink/>][link-git]。之后，
请按照以下命令clone项目并运行测试：
