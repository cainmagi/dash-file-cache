...
| `callback_on_exit` | `Callable[[IO[Any]], None] \| None` |                                   | 离开上下文时调用的回调函数。该函数会在文件被截断或关闭之前调用。                  |
...
### <IconObjType type="term"/> 离开上下文时，自动截断文件为空 {#-automatically-cut-the-file-to-an-empty-file-when-exiting-from-the-context}
